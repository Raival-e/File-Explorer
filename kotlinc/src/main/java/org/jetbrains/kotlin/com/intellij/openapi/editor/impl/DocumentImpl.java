...
//    });
     ...
     */
    @NonNull
    static RangeMarker createRangeMarkerForVirtualFile(@NonNull VirtualFile file,
                                                       int offset,
                                                       int startLine,
                                                       int startCol,
                                                       int endLine,
...
//    return marker;
     ...
     */
    boolean stripTrailingSpaces(@Nullable final Project project, boolean inChangedLinesOnly, int[] caretOffsets) {
        throw new UnsupportedOperationException();
...
//  @ApiStatus.Experimental
    ...
    private void throwGuardedFragment(@NonNull RangeMarker guard, int offset, @NonNull CharSequence oldString, @NonNull CharSequence newString) {
        if (myCheckGuardedBlocks > 0 && !myGuardsSuppressed) {
            DocumentEvent
                    event = new DocumentEventImpl(this, offset, oldString, newString, myModificationStamp, false, offset, oldString.length(),
                    offset);
